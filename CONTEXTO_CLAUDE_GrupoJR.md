...
13. **JS do link externo** requer `const token` e `function getCookie` definidos no início do bloco `<script>` — sem isso os fetches AJAX falham silenciosamente
14. **Fotos de veículos escoltados** usam momento fixo `antes` — a seção "após" foi removida da interface do agente
15. **Placas no link externo** têm máscara automática `AAA-0000` via função `mascaraPlaca()` — ao editar inputs de placa, sempre manter o `oninput="mascaraPlaca(this)"` e `maxlength="8"`
