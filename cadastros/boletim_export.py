...
_ROSA       = "FFB3BA"   # col I – Data Hora Encerramento
_ROSA_CLR   = "FFD6DA"   # dados col I
_AMARELO    = "FFFF00"   # col J – Total de Horas
_AMAR_CLR   = "FFFACD"   # dados col J
_LARANJA    = "FF8C00"   # header cols X e AA – TOTAL km/h
_LARA_CLR   = "FFF0D0"   # dados cols X e AA
_VERDE_TOT  = "1A6B35"   # linha VALOR BRUTO / col TOTAL
_FUNDO_TOT  = "D6E4F0"   # linha de totais
_CINZA_ROD  = "888888"

# ─── Definição das 30 colunas (A–AD) ────────────────────────────────────────
...
_COLUNAS = [
    ...
    ("Prestador\nServiço",              10,  14, 'L'),   #  2  C
    ("Placa",                            5,   8, 'C'),   #  3  D
    ("Veículo\nEscoltado",               8,  10, 'C'),   #  4  E
    # Bloco 2 — Datas
    ("Data Hora\nAgendamento VTR",      11,  18, 'C'),   #  5  F
    ("Data Hora\nChegada VTR",          10,  13, 'C'),   #  6  G
    ("Data Hora\nInício",               10,  13, 'C'),   #  7  H
    ("Data Hora\nEncerramento",         10,  13, 'C'),   #  8  I → ROSA
    ("Total de\nHoras",                  7,   9, 'C'),   #  9  J → AMARELO
    # Bloco 3 — Hodômetros
    ("Base\nKm",                         6,  13, 'R'),   # 10  K
    ("Hodômetro\nInicial",               7,  10, 'R'),   # 11  L
    ("Hodômetro\nFinal",                 7,  13, 'R'),   # 12  M
    ...
    ("Cidade\nOrigem",                  10,  14, 'L'),   # 14  O
    ("UF",                               3,   4, 'C'),   # 15  P
    ("Cidade\nDestino",                 10,  14, 'L'),   # 16  Q
    ("UF",                               3,   4, 'C'),   # 17  R
    # Bloco 5 — Franquia
    ("Valor Franquia\nContratada (R$)",  9,  13, 'R'),   # 18  S
    ("Franquia\nContratada de Km",       7,  10, 'R'),   # 19  T
    ("Franquia\nContratada de HORAS",    7,  13, 'C'),   # 20  U
    # Bloco 6 — KM Excedente
    ("KM\nexcedente",                    5,   8, 'R'),   # 21  V
    ("$ POR km\nexcedente",              6,   9, 'R'),   # 22  W
    ("TOTAL",                            6,  13, 'R'),   # 23  X → LARANJA
    # Bloco 7 — Horas Excedentes
    ("Horas\nexcedentes",                6,  13, 'C'),   # 24  Y
...
_IDX_ROSA  = 8    # I – Data Hora Encerramento
_IDX_AMAR  = 9    # J – Total de Horas
_IDX_LARA1 = 23   # X – TOTAL km excedente
_IDX_LARA2 = 26   # AA – TOTAL horas excedente
_IDX_TOTAL = 28   # AC – TOTAL geral

# Índices de colunas monetárias (R$) — 0-based
_IDX_MOEDA = {18, 22, 23, 25, 26, 27, 28}
...
def _missao_to_row(m: dict) -> list:
    """Converte dict da missão para lista de 30 valores (mesma ordem de _COLUNAS)."""
    return [
        m.get('os',          ''),    #  0  A
        m.get('cliente',     ''),    #  1  B
        m.get('prestador',   ''),    #  2  C
        m.get('placa',       ''),    #  3  D
        m.get('escoltado',   ''),    #  4  E
        m.get('agendamento', ''),    #  5  F
        m.get('chegada_vtr', ''),    #  6  G
        m.get('inicio_op',   ''),    #  7  H
        m.get('termino_op',  ''),    #  8  I ← ROSA
        m.get('total_h',     ''),    #  9  J ← AMARELO
        m.get('base_km',     ''),    # 10  K
        m.get('hod_ini',     ''),    # 11  L
        m.get('hod_fim',     ''),    # 12  M
        m.get('total_km',    0),     # 13  N
        m.get('cidade_ori',  ''),    # 14  O
        m.get('uf_ori',      ''),    # 15  P
        m.get('cidade_dst',  ''),    # 16  Q
        m.get('uf_dst',      ''),    # 17  R
        m.get('valor_franq', 0.0),   # 18  S
        m.get('franq_km',    0),     # 19  T
        m.get('franq_horas', ''),    # 20  U
        m.get('exced_km',    0),     # 21  V
        m.get('taxa_km',     0.0),   # 22  W
        m.get('subtotal_km', 0.0),   # 23  X ← LARANJA
        m.get('exced_h',     ''),    # 24  Y
        m.get('taxa_hora',   0.0),   # 25  Z
        m.get('subtotal_h',  0.0),   # 26  AA ← LARANJA
        m.get('despesas',    0.0),   # 27  AB
        m.get('total',       0.0),   # 28  AC ← TOTAL
        m.get('obs',         ''),    # 29  AD
    ]
...
def gerar_pdf_bytes(cliente: str, periodo: str, missoes: list, totais: dict) -> bytes:
    ...
    st_bLA    = _st('bLA', 5, True,  TA_LEFT,  c_azul, 6)
    ...
    tot_vals[0]  = Paragraph(f"<b>TOTAL — {n_regs:03d} OS</b>", st_bLA)
    tot_vals[9]  = Paragraph(f"<b>{totais.get('total_h','')}</b>",   st_bR)
    tot_vals[13] = Paragraph(f"<b>{totais.get('total_km','')}</b>",  st_bR)
    tot_vals[21] = Paragraph(f"<b>{totais.get('exced_km','')}</b>",  st_bR)
    tot_vals[23] = _tv('subtotal_km')
    tot_vals[24] = Paragraph(f"<b>{totais.get('exced_h','')}</b>",   st_bR)
    tot_vals[26] = _tv('subtotal_h')
    tot_vals[27] = _tv('despesas')
    tot_vals[28] = _tv('total')
    rows.append(tot_vals)

    # VALOR BRUTO
    vb_row = [''] * len(_COLUNAS)
    vb_row[27] = Paragraph("<b>VALOR BRUTO</b>",
                           _st('vbl', 6, True, TA_RIGHT, c_vtot, 7))
    vb_row[28] = Paragraph(f"<b>R$ {_fmt_brl(totais.get('total', 0))}</b>",
                           _st('vbv', 6, True, TA_RIGHT, c_vtot, 7))
    rows.append(vb_row)

    tabela = Table(rows, colWidths=col_w, repeatRows=1)
    n_data = len(rows) - 3  # sem header, totais e valor bruto
    ...
    cmds = [
        ...
        # Reaplicar cores especiais no header
...
def gerar_xlsx_bytes(cliente: str, periodo: str, missoes: list, totais: dict) -> bytes:
    ...
    ws.merge_cells(f'Q2:{LC}2')
    ws['Q2'] = f"Período: {periodo}  |  {n_regs:03d} registros"
    ws['Q2'].font      = _font(size=8, color=_AZUL_HDR)
    ws['Q2'].alignment = _align('right')
    ws['Q2'].fill      = _fill('EBF2FA')
    ...
    for ri, m in enumerate(missoes, start=HDR_ROW + 1):
        ...
        for ci, val in enumerate(vals, start=1):
            ...
            is_hora = (ci in {_IDX_AMAR + 1, 21})  # total_h (J) e franq_horas (U)
            if is_hora and val == '':
                val = '00:00'
    ...
    tot_map = {
        ...
        24: totais.get('subtotal_km', 0),
        25: totais.get('exced_h', ''),
        27: totais.get('subtotal_h', 0),
        28: totais.get('despesas', 0),
        29: totais.get('total', 0),
    }
    ...
    # ── VALOR BRUTO ────────────────────────────────────────────────────────
    VB_ROW = TOT_ROW + 1
    # A:AB merged → texto "VALOR BRUTO"; AC → valor; AD → vazio
    ws.merge_cells(f'A{VB_ROW}:AB{VB_ROW}')
    ...
    vb_cel = ws.cell(row=VB_ROW, column=N - 1, value=totais.get('total', 0))  # col AC = 29
    vb_cel.font          = _font(bold=True, size=10, color=_BRANCO)
    vb_cel.fill          = _fill(_VERDE_TOT)
    vb_cel.alignment     = _align('right')
    vb_cel.number_format = '#,##0.00'
    ws.row_dimensions[VB_ROW].height = 24

    # ── Rodapé ────────────────────────────────────────────────────────────
    ROD = VB_ROW + 1
    # Merge A:AC (sem Observações)
    ws.merge_cells(f'A{ROD}:AC{ROD}')
