...
def agente_certidao_tjdf(request, pk):
    ...
    try:
        ...
        if code == 200 and data.get('data'):
            # Verificar se há pendências nos registros retornados
            registros = data['data']
            tem_pendencia = any(
        ...
        messages.error(request, f'Falha ao conectar com InfoSimples: {e}')
...
def boletim_list(request):
    ...
    boletins = BoletimMedicao.objects.select_related('os', 'os__cliente', 'os__equipe', 'tabela_preco').prefetch_related('os__veiculos').order_by('-os__numero')
